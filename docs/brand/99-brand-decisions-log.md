...
**Implemented — 2026-09-17:** the picker ships in the app ("App Icon" row on Home → sheet with "People" and "Just for fun"). App-icon sets, picker previews, the Swift list and the alternate-icon build setting are all generated from `VARIANTS` by `scripts/brand/generate-brand-assets.py`; app icons are rendered without the grain layer to keep the 33 icons at 3.4 MB. Changing the set is one `VARIANTS` entry, a regenerate, and the `AppIcon.<name>` label in en/es/pt-BR. Still open: dark/tinted/clear appearances per variant.

## BD-039 — Wordmark and lock-ups

**Status:** Working  
**Date:** 2026-09-17  
**Direction:** The wordmark is drawn, not typeset: circular bowls, straight stems, one uniform stroke with round caps, and a capital C that is the icon's frame (rounded square, right corners stopped at 52°). Recommended primary lock-up: the **integrated** form in which the symbol is the C ("[symbol]alipic"); horizontal, stacked and wordmark-only forms exist for other spaces. The descriptor "ID Photos" is set as text, localisable, never drawn into the mark. The wordmark never appears inside the app icon and its letterforms are never used as a typeface.  
**Assets:** `docs/brand/assets/calipic-wordmark.svg`, `calipic-lockup-*.svg`, generated by `scripts/brand/build-wordmark.py`.  
**Details:** `docs/brand/prototypes/09-wordmark.md`. **Open:** founder choice of primary lock-up and stroke weight; designer's optical pass.

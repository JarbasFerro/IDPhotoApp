...
**Current set (33, second pass 2026-09-17):** people — swept (default), short, bald, spiky, curly, afro, bob, bun, spacebuns, ponytail, pigtails, braids, cap, beanie, hat, locs, turban, hijab, glasses, headphones, graduate; fun — cat, bunny, bear, fox, panda, frog, dinosaur, astronaut, robot, alien, party, viking. Characters that did not read at Home Screen size were removed at the founder's request (long, covered, unicorn, dog, mohawk, crown, beard, child, baby). **Rule:** a character stays only if it is identifiable at 180 px. Generated from `VARIANTS` in `scripts/brand/build-icon-v2.py`; adding one is a single table entry, optionally with negative-space cut-outs.  
**Constraints:** every character sits on the same shoulders, size and base line inside the same frame; no colour choice; no skin and no realistic facial features (simple negative-space cut-outs such as glasses, a visor or animal eye patches are allowed); BD-023 guardrails (no Face ID / surveillance / Contacts look) apply; the picker lives in settings/about, never in the capture → check → print path; App Store, marketing and lock-ups always use the default icon; every variant must ship the same dark/tinted/clear appearances as the default.  

...
| B. Moments | 0.10 | Done in 0.10.0: the portrait lands into its frame on Photo Check (wide and uncropped while checking, then the crop springs to the solved framing and the white background fades in, with a light haptic); the readiness ring around the shutter (four arcs that turn green, close into a full ring when ready, and host the 2-1 countdown; the shutter turns green and shows a tick while capturing); the sheet preview re-flows with a spring when copies or paper change (each placement is a view keyed by item and copy); "Your files are ready" with a bouncing tick and a success haptic on Share; hold-to-compare from stage A. Every animation is off under Reduce Motion. Device feedback on 0.10.0 led to 0.10.1: the landing is now explicit (the screen always opens uncropped, waits for the push to settle and the check to finish, then frames; before, the analysis finished during the push so nothing moved); the ring's arcs carry their icons, a one-time callout explains them and a "?" button reopens a help sheet; the countdown is 3-2-1 with the hint "Look at the lens"; sheet placements are keyed by person and copy so they glide instead of fading; "Your files are ready" is the Share headline with a large tick. Third pass (0.10.2) from the next device round: the forward landing jumped because the white-background layer was inserted at its final geometry, so both layers now stay mounted and only crossfade; the ring reveals its four checks one at a time in hint order (framing, distance, head, light) with icons that bounce when they turn green; the help sheet opens on every camera launch until "Don't show this again"; Auto is off by default with a callout under its button; Share's digital photos are tappable thumbnails with a share badge, buttons are regular height, and counts use grammatical inflection. Fourth pass (0.10.4): the instructions come before the camera as a two-page screen (the ring with a colour legend and the four checks; then Auto with its own switch), full screen on every launch until "Don't show this again", and the camera only starts after it; the "?" and Auto controls sit on the top line at one height and the hint moves below them, larger; Photo Check fits one screen with the portrait taking the leftover height and the actions in a bottom bar; the sheet summary's plural markup is rendered through AttributedString; Share says "Share JPEG" for the lab pages and drops the caption under a single thumbnail | Does each moment explain something? Nothing flickers or lags on the phone |

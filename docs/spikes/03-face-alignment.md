...
Covered by unit tests: crown agreement/hair-volume/bad-mask/cut-off/no-mask branches; well-framed face lands on the 74 % / 42 % targets and round-trips through the editor model; tilts of 4° and 12° are levelled (12° still flagged) and 35° is reported without rotation; a 35° head yields the same crop size as an upright one and the crop contains eyes, chin, and crown; a small face fails resolution, hits the zoom limit, and fails overall; a face near the top keeps the crop inside the source; multiple faces fail, hair volume becomes a manual check, no mask becomes a warning; rotation clamps to ±15° and invalid values reset to 0; solutions are deterministic.

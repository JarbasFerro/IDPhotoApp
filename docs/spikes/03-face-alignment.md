...
**Status:** Domain solver, Vision adapter, automatic alignment in the editor, and a macOS fixture harness implemented. Real-face evidence comes from macOS Vision on five private portraits; iOS simulators on this Mac cannot run Vision inference, so on-device iOS evidence is still pending.  
...
- `App/Domain/FaceGeometry.swift` — `FaceGeometry` (top-left normalized points plus source pixel size so distances are computed in pixels), `CrownEstimator` (crown = chin − k·(chin − eye line), k = 1.8 for adults; the person-mask top wins when it agrees within 0.35 inter-eye distances, otherwise anatomy wins and a hair-volume or head-cut-off flag is raised), `CompositionSpec` (ICAO Portrait Quality engineering defaults: head 60–90 % of height with 74 % target, eye midpoint 30–50 % from top with 42 % target, ±5° yaw/pitch, ±8° roll, IED ≥ 90 px with 120 px recommended), and `CropSolver` (level eyes within the roll limit, scale from head height, place by eye line and headroom, centre horizontally, express as the editor's zoom/travel/rotation model, emit `pass / warn / fail / manualCheck` checks).
- `App/Domain/PhotoGeometry.swift` — `CropAdjustment.rotationDegrees` (±8°, counter-clockwise about the crop centre), applied in the preview and in the export renderer.
- `App/Imaging/FaceAnalyzer.swift` — `DetectFaceLandmarksRequest(.revision3)` pinned per ADR-040; pupils (eye regions as fallback) and the lowest face-contour point converted with `pointsInImageCoordinates(_:origin: .upperLeft)`; eyes ordered by x because Vision names them from the viewer's side; eye-line roll computed from the pupils; `GeneratePersonSegmentationRequest` at `.accurate` (falls back to `.balanced`, then to no mask) sampled in a column band ±0.6 IED around the eye midpoint for the mask crown. Segmentation is a prior, never a dependency.
...
- `App/Features/ContentView.swift`, `AlignmentPresentation.swift` — status card with symbol and text per check (never colour alone), "Align automatically" replacing Reset when a solution exists, a Straighten slider, and an explicit note when analysis is unavailable on the device.
...
- Fixtures: five webcam portraits of one consenting adult, 1440 × 960, short hair, no glasses, camera below eye level; stored locally only (ADR-027).

## Validation evidence

| Check | Result |
|---|---|
| Domain and pipeline suites (iOS 26.0 and 27.0 simulators) | 36 Swift Testing tests pass; the private-fixture test records one known issue because Vision cannot create an inference context on these simulators |
...
| macOS harness on five portraits | Every picture: one face, mask crown chosen with confidence 0.9, mask/anatomy divergence within ±0.15 IED, head 74 % and eyes 42 % as targeted, analysis 50–70 ms per picture after a 2 s model load |
...
|---|---|---|---|---|---|---|---|---|---|
| 0 | 138 | −10.5 / −6.5 | 9.3 | 7.3 | 0.01 | 1.79 | 0.0 | warn | roll, yaw, pitch |
| 1 | 97 | −4.2 / −0.4 | 5.1 | −2.9 | −0.14 | 1.87 | 4.2 | warn | resolution (warn), yaw |
| 2 | 192 | −2.8 / 0.7 | 2.0 | 16.0 | −0.15 | 1.88 | 2.8 | warn | pitch, headroom (warn) |
| 3 | 133 | −1.4 / 0.9 | 8.3 | −0.2 | 0.11 | 1.74 | 1.4 | warn | yaw |
| 4 | 59 | −2.2 / 1.6 | 1.6 | 4.4 | −0.09 | 1.85 | 2.2 | fail | resolution (fail) |

Visual review of the annotated previews and aligned crops: the mask crown sits on the top of the hair; the pupils are on the irises; the chin point sits 2–4 % of head height below the visible chin (on the neck shadow), which slightly enlarges the head estimate; the aligned 26 × 32 crops are credible ID compositions with levelled eyes.
...
Fixed during the spike: eye ordering (Vision's "left eye" is image-left, so the initial roll came out near 180°); mask scan direction (bitmap memory row 0 is the top scanline, so the first pass reported every mask as touching the top edge).

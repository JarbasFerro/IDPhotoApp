...
- **Auto capture** (toggle "Auto", off by default since 0.10.2, remembered): three seconds of "Ready" with a visible 3, 2, 1 countdown inside the shutter ring, the hint "Look at the lens", and a tick per second; any hint change cancels it. The ring reveals its four checks one at a time in hint order; a help sheet explains them on launch until dismissed for good.
